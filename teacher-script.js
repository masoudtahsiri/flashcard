...
function showTab(tabName) {
    ...
    } else if (tabName === 'bulkImport') {
        document.getElementById('bulkImportContent').classList.add('active');
        document.getElementById('bulkImportTab').classList.add('active');
        // Populate category dropdown for bulk import
        populateBulkImportCategories();
...
function validateImportData(data) {
    ...
    data.forEach((item, index) => {
        ...
        if (!item.word || typeof item.word !== 'string' || item.word.trim() === '') {
            errors.push(`Row ${rowNum}: Missing or invalid word`);
        }
        
        // Check optional fields
        if (item.image_url && typeof item.image_url !== 'string') {
            warnings.push(`Row ${rowNum}: image_url should be a string`);
        }
        
        if (item.category && typeof item.category !== 'string') {
            warnings.push(`Row ${rowNum}: category should be a string`);
        }
        ...
        const duplicateIndex = data.findIndex((otherItem, otherIndex) => 
            otherIndex !== index && 
            otherItem.word && 
            otherItem.word.trim().toLowerCase() === item.word.trim().toLowerCase()
        );
        
        if (duplicateIndex !== -1) {
            warnings.push(`Row ${rowNum}: Duplicate word "${item.word}" (also found in row ${duplicateIndex + 1})`);
        }
    });
...
function createPlaceholderImage(word) {
    ...
    ctx.fillText(word, 200, 150);
    
    return canvas.toDataURL('image/png');
}

// Function to preview bulk import
async function previewBulkImport() {
    ...
    try {
        ...
        // Create preview with image processing
        bulkImportPreview = [];
        const previewContent = document.getElementById('previewContent');
        previewContent.innerHTML = '<div class="loading-state">Processing images...</div>';
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (item.image_url) {
                imageData = await convertImageToBase64(item.image_url);
            }
            
            if (!imageData) {
                imageData = createPlaceholderImage(item.word);
            }
            
            bulkImportPreview.push({
                word: item.word.trim(),
                image: imageData,
...
// Function to render import preview
function renderImportPreview() {
    const previewContent = document.getElementById('previewContent');
    const totalCardsPreview = document.getElementById('totalCardsPreview');
    
    previewContent.innerHTML = bulkImportPreview.map((item, index) => `
        <div class="preview-item">
            <img src="${item.image}" alt="${item.word}" onerror="this.src='data:image/svg+xml;base64,'">
            <div class="preview-text">${item.word}</div>
            <div class="preview-category">${item.category || 'No category'}</div>
        </div>
    `).join('');
    
    totalCardsPreview.textContent = bulkImportPreview.length;
...
// Function to populate bulk import category dropdown
function populateBulkImportCategories() {
    const categorySelect = document.getElementById('bulkImportCategory');
...
// Function to execute bulk import
async function executeBulkImport() {
    ...
    const defaultCategoryId = document.getElementById('bulkImportCategory').value;
    const overwriteExisting = document.getElementById('bulkImportOverwrite').checked;
    ...
    for (let i = 0; i < bulkImportPreview.length; i++) {
        const item = bulkImportPreview[i];
        const progress = ((i + 1) / bulkImportPreview.length) * 100;
        
        progressFill.style.width = progress + '%';
        progressText.textContent = `Importing ${i + 1} of ${bulkImportPreview.length}: ${item.word}`;
        
        try {
            // Find category ID
            let categoryId = null;
            if (item.category) {
                const category = groups.find(g => g.name.toLowerCase() === item.category.toLowerCase());
                if (category) {
                    categoryId = category.id;
                } else {
                    results.warnings.push(`"${item.word}": Category "${item.category}" not found, using default`);
                    categoryId = defaultCategoryId ? parseInt(defaultCategoryId) : null;
                }
            } else {
                categoryId = defaultCategoryId ? parseInt(defaultCategoryId) : null;
            ...
            const existingCard = flashcards.find(card => 
                card.word.toLowerCase() === item.word.toLowerCase()
            );
            
            if (existingCard && !overwriteExisting) {
                results.warnings.push(`"${item.word}": Card already exists, skipping`);
                continue;
            }
            ...
            // Create new card
            const newCard = {
                id: existingCard ? existingCard.id : nextId++,
                word: item.word,
                image: item.image,
                audioUrl: '',
                categoryId: categoryId,
                sortOrder: existingCard ? existingCard.sortOrder : maxSortOrderInCategory + 1
            };
            
            // Add or update card
            if (existingCard && overwriteExisting) {
                // Update existing card
                const cardIndex = flashcards.findIndex(card => card.id === existingCard.id);
                flashcards[cardIndex] = { ...existingCard, ...newCard };
                results.success.push(`"${item.word}": Updated existing card`);
            } else {
                // Add new card
                flashcards.push(newCard);
                results.success.push(`"${item.word}": Added new card`);
            }
            ...
            results.errors.push(`"${item.word}": ${error.message}`);
            console.error('Import error for', item.word, ':', error);
        }
    }
    ...
    // Render results
    resultsContent.innerHTML = [
        ...results.success.map(msg => `<div class="result-item result-success">✓ ${msg}</div>`),
        ...results.warnings.map(msg => `<div class="result-item result-warning">⚠ ${msg}</div>`),
        ...results.errors.map(msg => `<div class="result-item result-error">✗ ${msg}</div>`)
    ].join('');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('bulkImportFile').addEventListener('change', function() {
    ...
    document.getElementById('previewImportBtn').addEventListener('click', previewBulkImport);
    document.getElementById('executeImportBtn').addEventListener('click', executeBulkImport);
