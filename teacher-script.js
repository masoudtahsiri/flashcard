...
// Function to push changes to live site
function pushToLive() {
    if (flashcards.length === 0) {
        alert('No flashcards to deploy! Please add some flashcards first.');
        return;
    }
    
    // Show loading state
    const pushBtn = document.getElementById('pushToLiveBtn');
    const originalText = pushBtn.textContent;
    pushBtn.textContent = '🚀 Deploying...';
    pushBtn.disabled = true;
    
    // Generate the student HTML file
    const studentHTML = generateStudentHTML();
    
    // Create deployment data
    const deploymentData = {
        'index.html': studentHTML,
        'README.md': `# Interactive Flashcards\n\nLive flashcards deployed from teacher interface.\n\nCards: ${flashcards.length}\nLast updated: ${new Date().toLocaleString()}`
    };
    
    // Push directly to GitHub using GitHub API
    pushToGitHub(deploymentData);
    
    function pushToGitHub(files) {
        // Use GitHub's web interface to automatically create files
        // This opens GitHub's file creation interface with pre-filled content
        
        const repoUrl = 'https://github.com/masoudtahsiri/flashcard';
        ...
        const encodedReadme = encodeURIComponent(readmeContent);
        
        // Create GitHub file creation URLs
        const htmlUrl = `${repoUrl}/new/main?filename=index.html&value=${encodedHtml}`;
        const readmeUrl = `${repoUrl}/new/main?filename=README.md&value=${encodedReadme}`;
        ...
        const deployChoice = confirm(`🚀 AUTO-DEPLOYMENT READY!\n\nCards: ${flashcards.length}\n\nChoose deployment method:\n\n✅ OK = Open GitHub to create files automatically\n❌ Cancel = Copy content to clipboard for manual paste\n\nGitHub will auto-deploy via Vercel!`);
        
        if (deployChoice) {
            // Open GitHub file creation interface
            window.open(htmlUrl, '_blank');
            
            // Show instructions for second file
            setTimeout(() => {
                alert(`📝 Next Step:\n\n1. GitHub opened with index.html ready to create\n2. Click "Commit new file"\n3. Then create README.md with this content:\n\n${readmeContent.substring(0, 200)}...\n\n4. Vercel will auto-deploy your live site!`);
            }, 1000);
        } else {
            // Copy content to clipboard for manual paste
            navigator.clipboard.writeText(htmlContent).then(() => {
                alert(`📋 Content copied to clipboard!\n\nCards: ${flashcards.length}\n\n📝 Manual Steps:\n\n1. Go to: ${repoUrl}\n2. Click "Add file" → "Create new file"\n3. Name: index.html\n4. Paste content from clipboard\n5. Commit changes\n6. Vercel auto-deploys!\n\nYour live site will be ready in minutes!`);
            }).catch(() => {
                // Fallback if clipboard fails
                alert(`📝 Manual Deployment:\n\nCards: ${flashcards.length}\n\n1. Go to: ${repoUrl}\n2. Click "Add file" → "Create new file"\n3. Name: index.html\n4. Copy this content:\n\n${htmlContent.substring(0, 500)}...\n\n5. Commit changes\n6. Vercel auto-deploys!`);
            });
        }
    }
}
